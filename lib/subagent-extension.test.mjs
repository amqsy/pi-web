...
test("before_agent_start injects orchestrator guidance in multi-turn sessions", async () => {
  let handler;
  const extension = createSubagentExtension({
    async start() { throw new Error("unused"); },
    async get() { return null; },
  ...
  // First turn: no previous assistant messages
  const firstTurn = await handler(
    { systemPrompt: "base prompt" },
    { sessionManager: { getBranch: () => [{ type: "message", message: { role: "user" } }] } },
  );
  assert.equal(firstTurn, undefined);

  // Multi-turn: contains assistant messages
  const multiTurn = await handler(
    { systemPrompt: "base prompt" },
    {
      sessionManager: {
        getBranch: () => [
  ...
  assert.match(multiTurn.systemPrompt, /多轮会话指挥官强制守则/);
  assert.match(multiTurn.systemPrompt, /base prompt/);
});
